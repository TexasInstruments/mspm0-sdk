...
Bridge / translator devices convert between two comm interfaces(CAN, SPI). 
...
In the example, the code can be selected to work as SPI controller or peripheral.
Users can modify the Mode Select in the SysConfig file to switch modes. 
...
This example specifies the SPI message format.
When receiving the message from SPI, the message format is 
< 55 AA ID1 ID2 ID3 ID4 Length Data1 Data2 ...>. 
Users can send data to the CAN bus from the terminal by entering data as the same format. 
