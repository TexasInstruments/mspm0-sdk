...
Bridge / translator devices convert between two comm interfaces(CAN, SPI controller). 
CAN-SPI bridge allows a device to send/receive information 
...
another can-to-spi bridge project or using a CAN Bus Analyzer tool of your
choice.
...
It should be noted that SPI is a communication method that SPI controller control the transmit and receive. 
In general, SPI target cannot initiate communication. 
For SPI target-to-controller communication, SPI target will pull down the IO when messages need to be sent. 
SPI controller will initiate receive operation in IO interrupt when IO is detected low.

Compile, load and run the example.

CANFD Transmissions to the device will be passed on through SPI at 500kHz bit rate. 
SPI Transmissions to the device will be passed on through CANFD at 250k/2M speed.

For SPI, The example has the 500kHz bit rate.
...
For transparent transmission, a configurable timeout is used for the SPI slave to detect one message. 
Data from the SPI is filled into the data area of the CAN (same in reverse). The CAN ID is the default value.

For protocol transmission, this example specifies the SPI message format. 
Users can modify the format according to application requirements. 
When receiving the message from the SPI, the message format is < 55 AA ID1 ID2 ID3 ID4 Length Data1 Data2 ...> 
Users can send data through SPI with the same format. 55 AA is the header. The ID area is four bytes. 
The length area is one byte, which indicates the data length.

For more information about how to use this example to develop the bridge application as needed, 
please visit the [Bridge Design between CAN and SPI with MSPM0 MCUs](https://www.ti.com/lit/pdf/slaaen5).
