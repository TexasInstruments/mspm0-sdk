...
Bridge / translator devices convert between two comm interfaces(CAN, UART). 
CAN-UART bridge allows a device to send/receive information 
...
another can-to-uart bridge project or using a CAN Bus Analyzer tool of your
choice.

Connect UART RX and TX between two UART devices or to the Launchpad's back-channel UART.

Compile, load and run the example.

CANFD Transmissions to the device will be passed on through UART at 9600 baud. 
UART Transmissions to the device will be passed on through CANFD at 250k/2M speed.

For UART, The example has the 9600 baud.
...
For transparent transmission, a configurable timeout is used for UART to detect one message. 
Data from UART is filled into the data area of CAN (same in reverse). CAN ID is the default value.

For protocol transmission, this example specifies the UART message format. 
Users can also modify the format according to application requirements. 
When receiving the message from UART, the message format is < 55 AA ID1 ID2 ID3 ID4 Length Data1 Data2 ...>. 
Users can send data to the CAN bus from the terminal by entering data in the same format. 
55 AA is the header. ID area is four bytes. The length area is one byte, which indicates the data length.

For more information about how to use this example to develop the bridge application as needed, 
please visit the [Bridge Design between CAN and UART with MSPM0 MCUs](https://www.ti.com/lit/pdf/slaaen4).
